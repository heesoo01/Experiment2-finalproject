...
본인이 제안한 부분은 `DH_FR1.mat` 데이터셋의 구조를 먼저 분석하고, 그 구조에 맞게 알고리즘을 재설계한 점이다. 학습 데이터에는 정답 위치 `p`가 있으므로, `BS_positions`와 `p`를 이용해 실제 기하학적 거리 `d_true = ||p - b_i||`를 계산할 수 있다. 이를 `d_hat`과 비교한 결과, `d_hat`은 실제 거리와 큰 차이를 보였고 평균 절대 거리 오차가 약 16.17 m 수준으로 나타났다. 이 분석을 통해 본 문제를 clean distance 기반 삼변측량 문제가 아니라, NLOS와 bias가 포함된 noisy RTT fingerprinting 문제로 해석하였다.
